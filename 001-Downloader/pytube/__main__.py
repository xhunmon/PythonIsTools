...
class YouTube(Downloader):
    ...
    def on_progress_callback(self, stream, chunk, bytes_remaining):
        print("进度: %d" % (bytes_remaining))
        Downloader.print_ui(txt="%s-->进度: %d" % (self.title, bytes_remaining))

    def on_complete_callback(self, any, file_path):
        print("%s; " % self.title)
        Downloader.print_ui(txt="%s 100%%" % self.title)
        Downloader.add_success_count()

    def start(self, url, path):
        self.register_on_complete_callback(self.on_complete_callback)
        self.register_on_progress_callback(self.on_progress_callback)
        Downloader.add_total_count()
        Downloader.add_downloading_count()
        Downloader.print_ui(txt="开始任务：%s" % url)
        try:
            # dl = self.streams.first()
            # dl = self.streams.filter(progressive=True, file_extension='mp4').order_by('resolution').desc().first()
            dl = self.streams.filter(progressive=True, file_extension='mp4').order_by('fps').asc().first()
            Downloader.print_ui(txt="准备下载：%s" % self.title)
            dl.download(output_path=path)
        except Exception as e:
            Downloader.print_ui(txt="%s 下载失败!" % self.title)
