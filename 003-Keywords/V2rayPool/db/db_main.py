...
from V2rayPool.core import client
from V2rayPool.db.local import DbLocal, DbEnable
from V2rayPool.db.net import *

dbLocal = DbLocal()
check = PYCheck()
dbEnable = DbEnable().get()


def __add_urls_de_dup(all_urls: [], new_urls: []) -> []:
    """合并数组，并且去重，去空"""
...
def start_random_v2ray_by_local():
    """从本地随机启动一个可用的proxy"""
...
def load_urls_and_save_auto():
    """首先通过不需要代理的网页获取节点，当代理有可用时，开启代理，获取需要代理获取的网页"""
    dbLocal.clear_local()
    all_urls = load_urls_by_not_proxy()
    proxy_url = None
    for url in all_urls:
    ...
    proxy_urls = load_urls_by_net_with_proxy(proxy_url=proxy_url)
    all_urls = all_urls + proxy_urls
    check_and_save(all_urls, append=False)


def load_urls_by_not_proxy(save_local=True):
    all_urls = []
    # 1. 先把不需要代理的先请求下来
    __add_urls_de_dup(all_urls, PNSsfree().get_urls())
    print("获取https://view.ssfree.ru/后数目：%d" % len(all_urls))
    __add_urls_de_dup(all_urls, PNFreevpnX().get_urls())
    print("获取https://freevpn-x.com/后数目：%d" % len(all_urls))
    __add_urls_de_dup(all_urls, PNGithubIwxf().get_urls())
    print("获取https://github.com/iwxf/free-v2ray/blob/master/README.md 后数目：%d" % len(all_urls))
    __add_urls_de_dup(all_urls, PNFreeV2ray().get_urls())
    print("获取https://view.freev2ray.org/ 后数目：%d" % len(all_urls))
    if save_local:  # 保存到本地
        dbLocal.save_urls(all_urls)
    return all_urls


def load_urls_by_net_with_proxy(proxy_url=None, save_local=True):
    all_urls = []
    if save_local:  # 保存到本地
        dbLocal.save_urls(all_urls)
    if not proxy_url:
...
def load_urls_by_net(proxy_url=None, save_local=True, need_proxy=True):
    """
    通过网络获取最新的节点，但是需要代理
    ...
    print("获取从https://freefq.com/ 后数目：%d" % len(all_urls))
    if save_local:  # 保存到本地
        dbLocal.save_urls(all_urls, append=False)
    return all_urls


def load_unchecked_urls_by_local():
    """获取本地未校验过的url"""
    dbLocal.get_urls(False)
    urls = dbLocal.get_checked_urls()
    return urls


def load_enable_urls_by_local():
    """获取已检测过的url"""
    return dbEnable.get_urls()


def check_url_single(url: str):
...
def check_and_save(urls: [], append=True):
    """检测url是否可用，并且保存到本地"""
    if not append:
        dbEnable.clear_local()
    all_infos = dbEnable.get_infos()
    new_infos = []
    size = len(urls)
    for i in range(size):
        try:
            if i % 30 == 0:  # 每三十个更新一次
                dbLocal.save_urls(urls=urls, append=False)  # 更新剩下的
            url = urls.pop()
            in_all = False
            ...
            dbEnable.save_urls(new_infos)  # 写入已通过的
            new_infos.clear()
        except Exception as e:
            print(e)
    # 最后
    print('%s！总共：%d |待检测：%d |可用：%d' % ('全部检测完毕！', size, len(urls), len(all_infos)))
    dbEnable.save_urls(new_infos)
    dbLocal.clear_local()
